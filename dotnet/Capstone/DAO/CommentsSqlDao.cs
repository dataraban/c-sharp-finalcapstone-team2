...
namespace Capstone.DAO
{
    public class CommentsSqlDao : ICommentsDao
    {
        ...
        public List<string> PostComment(int userId1, int userId2, string comment)
        {
            List<string> comments = new List<string>();
            try
            {
                ...
                {
                    conn.Open();
                    SqlCommand cmd = new SqlCommand("INSERT INTO user_comments (user_author_id, user_posted_id, comment_text) VALUES (@user_author_id, @user_posted_id, @comment_text);", conn);
                        cmd.Parameters.AddWithValue("@user_author_id", userId1);
                        cmd.Parameters.AddWithValue("@user_posted_id", userId2 );
                        cmd.Parameters.AddWithValue("@comment_text", comment);
                    SqlDataReader reader = cmd.ExecuteReader();
                    while (reader.Read())
                    {
                        string newcomment = Convert.ToString(reader["comment_text"]);
